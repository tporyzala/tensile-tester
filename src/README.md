...
The Set XLSX link downloads one workbook from `/api/test/samples/csv`. The route name is historical; the response is an `.xlsx` file named `tensile-sample-set.xlsx`. The workbook opens with a Summary worksheet containing one row per sample, included-sample statistics, and peak-force/final-displacement charts for all recorded samples. A Method worksheet shows the saved or latest known editable method snapshot, and a Plots worksheet includes a sampled force-displacement overlay for every sample with telemetry. Chart source data remains visible by default so Excel renders plot lines without hidden-data chart options. Each sample still gets its own raw telemetry worksheet named from the automatic sample number, with Excel-invalid worksheet characters replaced and duplicate names made unique. Telemetry columns and simple numeric metadata are written as spreadsheet numbers, while sample IDs, notes, statuses, and method metadata remain text. The workbook includes the method ID, method name, method hash, and full method snapshot used when that sample started.
...
- include that stopped sample in overlays and statistics by default
...
STOPPED   operator stopped the run; partial sample is retained and included by default
