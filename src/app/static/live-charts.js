...
class LiveForceCharts {
  ...
  appendPoints(points) {
    ...
    this.livePoints.push(...normalized);
    this.render();
  }

  setLivePoints(points) {
    this.livePoints = normalizePlotPoints(points);
    this.render();
  }
  ...
  timeTraces() {
    ...
    const commandedY = this.livePoints.map((point) => point.commandedForceN);
    const hasCommanded = commandedY.some((value) => Number.isFinite(value));
    if (!hasCommanded) {
      return [measured];
    }
  ...
  displacementTraces() {
    ...
    for (let index = 0; index < this.overlaySeries.length; index += 1) {
      ...
      traces.push({
        ...
        marker: { color: colors[index % colors.length], size: 3 },
        opacity: 0.65,
        hovertemplate:
...
function normalizeOverlaySeries(series) {
  return {
    ...
    points: normalizeOverlayPoints(series && series.points),
  };
}
